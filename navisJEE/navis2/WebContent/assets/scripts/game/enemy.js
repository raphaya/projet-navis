...
var countMissile = 0;
function createEnemyMissile(sidePop, maxMissile) {
...
function createEnemyMotherDrone(sidePop) {
    ...
    if (enemyMotherDrone) {
        ...
        enemyMotherDrone.health = 300;
    }
    game.add.tween(enemiesMotherDrone).to({ y: 30 }, 1000, Phaser.Easing.Linear.None, true, 0, 1000, true).loop();
}

function createEnemyDrone(positionX) {
    if (livingDrones.length < 1) {
        ...
        if (enemyDrone) {
            enemyDrone.reset(positionX, 325, 'enemyDrone');
